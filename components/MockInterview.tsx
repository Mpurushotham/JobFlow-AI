...
export const MockInterview: React.FC<MockInterviewProps> = ({ profile, subscriptionTier }) => { // FIX: Exported MockInterview as a named export.
    ...
    const handleAnalyzeAnswer = useCallback(async () => {
        if (!profile.resumeContent) {
            addNotification("Please add your master resume in the Profile section to analyze your answer.", 'error');
            return;
        }
        if (!isAIPro) {
    ...
    }, [profile.resumeContent, isAIPro, transcript, currentQuestion, addNotification]); // Dependencies for useCallback

    useEffect(() => {
        if (!recognition || !isAIPro) return; // Only enable if AI Pro
    ...
    }, [transcript, addNotification, isAIPro, handleAnalyzeAnswer]); // Depend on handleAnalyzeAnswer
            ...
            <div className="w-full max-w-3xl space-y-8">
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700">
                    <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
                        <User size={20} className="text-purple-500" /> Mock Interview
                    </h3>
                    ...
                    <div className="p-6 bg-purple-50 dark:bg-purple-900/20 rounded-2xl border border-purple-100 dark:border-purple-800 relative">
                        ...
                        <p className="text-xl font-bold text-gray-800 dark:text-white mt-2 leading-relaxed">
                            {currentQuestion}
                        </p>
                    </div>
                    ...
                    <div className="mt-4 bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-800">
                        ...
                        >
                            ...
                            {showStarTip ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                        </button>
                        {showStarTip && (
                            <div className="mt-3 text-sm text-blue-900 dark:text-blue-300 leading-relaxed">
                    ...
                    <div className="mt-6 flex justify-between gap-4">
                        ...
                        >
                            {isListening ? <Mic size={18} className="animate-spin-fast" /> : <Mic size={18} />} {/* Added animate-spin-fast */}
                        ...
                        <button
                            ...
                            disabled={loading || !transcript.trim() || !isAIPro}
                ...
                {transcript && (
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 animate-fade-in">
                        <h4 className="text-lg font-bold text-gray-800 dark:text-white mb-4">Your Answer</h4>
                ...
                {feedback && (
                    <div className="bg-purple-50 dark:bg-purple-900/20 p-8 rounded-3xl shadow-sm border border-purple-100 dark:border-purple-800 animate-fade-in">
                        <h4 className="text-lg font-bold text-purple-800 dark:text-purple-300 mb-4 flex items-center gap-2">
                            <Info size={20} /> AI Feedback
                        </h4>
